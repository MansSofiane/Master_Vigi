...
//Calcule du nombre de page
    $rqtc = $bdd->prepare("SELECT * FROM `agence`  WHERE   id_user in (select id_user from utilisateurs where agence=(select agence from utilisateurs where id_user='$id_user')) and typ_agence='1' and lib_agence LIKE '%$rech%' ORDER BY `cod_agence` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage = ceil($nbe / 8);
//Pointeur de page
...
//requete à suivre
    ...
    $rqt = $bdd->prepare("SELECT * FROM `agence`  WHERE   id_user in (select id_user from utilisateurs where agence=(select agence from utilisateurs where id_user='$id_user')) and typ_agence='1'  and lib_agence LIKE '%$rech%' ORDER BY `cod_agence` DESC  LIMIT $part ,8");
    $rqt->execute();


}else{

    $rqtc = $bdd->prepare("SELECT * FROM `agence`  WHERE   id_user in (select id_user from utilisateurs where agence=(select agence from utilisateurs where id_user='$id_user')) and typ_agence='1'  ORDER BY `cod_agence` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage = ceil($nbe / 8);
//Pointeur de page
...
//requete à suivre
    ...
    $rqt = $bdd->prepare("SELECT * FROM `agence`  WHERE  id_user in (select id_user from utilisateurs where agence=(select agence from utilisateurs where id_user='$id_user')) and typ_agence='1'  ORDER BY `cod_agence` DESC  LIMIT $part ,8");
    $rqt->execute();
}
...
<div class="widget-box">
    <ul class="quick-actions">
        ...
        <li class="bg_lg"> <a onClick="Menu('avoy','produit/new_agence.php')"> <i class="icon-folder-open"></i>Nouvel-Apporteur </a> </li>
        <li class="bg_ly"> <a onClick="Menu('avoy','produit/list_agence.php')"> <i class="icon-folder-open"></i> Liste Apporteurs</a> </li>
