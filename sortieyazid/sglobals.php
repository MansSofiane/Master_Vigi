...
//Requete Direction R�gionales
if($agence=='0'){
//requete generale
$rqtg=$bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`pn`,d.`pt`,d.mtt_reg,d.mtt_solde,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod` ,s.`nom_sous`, s.`pnom_sous`,m.`lib_mpay`,u.`agence` FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user` AND d.`cod_prod`='$prod' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d')>='$date1' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d')<='$date2' ORDER BY u.`agence`");
$rqtg->execute();
...
if($typ_usr=='dr'){
//Requete DR
$rqtg=$bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`pn`,d.`pt`,d.mtt_reg,d.mtt_solde,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod` ,s.`nom_sous`, s.`pnom_sous`,m.`lib_mpay`,u.`agence` FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user` AND d.`cod_prod`='$prod' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d')>='$date1' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d')<='$date2' AND u.id_par='$agence' ORDER BY u.`agence`");
$rqtg->execute();
//requete pour les avenants positifs
...
//Requete Agence
$rqtg=$bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`pn`,d.`pt`,d.mtt_reg,d.mtt_solde,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod` ,s.`nom_sous`, s.`pnom_sous`,m.`lib_mpay`,u.`agence` FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user` AND u.`agence`='$cas_par_agence' AND d.`cod_prod`='$prod' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d')>='$date1' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d')<='$date2' ORDER BY u.`agence`");
$rqtg->execute();
//requete pour les avenants positifs
...
$pdf->Cell(400,10,'Bordereau de production  du '.date("d/m/Y", strtotime($date1)).' au '.date("d/m/Y", strtotime($date2)).'  --Document g�n�r� le-- '.date("d/m/Y", strtotime($datesys)) ,'1','1','L','1');
while ($row_p=$rqtp->fetch()){
$pdf->Cell(133,10,'DG-AGLIC','1','0','C');$pdf->Cell(133,10,'Produit: '.$row_p['lib_prod'],'1','0','C');$pdf->Cell(134,10,'Code produit: '.$row_p['code_prod'],'1','1','C');
	$lib_prod=$row_p['lib_prod'];
}
$pdf->Ln();
$pdf->SetFont('Arial','B',10);
	$pdf->Cell(50,5,'Police N�','1','0','C');$pdf->Cell(50,5,'Avenant N�','1','0','C');$pdf->Cell(60,5,'Nom&Pr�nom-R.Sociale','1','0','C');$pdf->Cell(45,5,'P.Nette','1','0','C');$pdf->Cell(30,5,'C.Police','1','0','C');$pdf->Cell(40,5,'P.Commerciale','1','0','C');$pdf->Cell(25,5,'D.Timbre','1','0','C');$pdf->Cell(45,5,'P.Total','1','0','C');$pdf->Cell(30,5,'Reglement','1','0','C');$pdf->Cell(30,5,'Solde','1','0','C');
$pdf->Ln();
//Boucle police
