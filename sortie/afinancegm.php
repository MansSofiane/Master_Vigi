...
*/
    ...
    while ($row_p=$rqtp->fetch()){
        ...
        if($user_selected!=0)
            $pdf->Cell(405,10,'Agence N°: '.$agence,'1','0','L');
        else
    ...
    $pdf->Cell(57, 10, 'Produit', '1', '0', 'C');
    $pdf->Cell(10, 10, 'Sens', '1', '0', 'C');
    ...
    while ($rp=$rqtp->fetch()) {
        ...
        if($user_selected==0){
            ...
            $rqtavneg=$bdd->prepare("select  v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,a.dat_avis,v.dat_eff,v.dat_ech,
                                              s.nom_sous,s.pnom_sous,a.mtt_avis,a.cod_ref,a.cod_av,a.base_pn_commission,a.mtt_cpl,a.mtt_dt,v.lib_mpay
                                    from avis_recette  a, policew as p ,souscripteurw as s,avenantw as v,utilisateurs u
            ...
            $rqt=$bdd->prepare("select  p.cod_pol police,'0' as avenant,p.cod_prod,p.sequence sequence_police,'' sequence_av,p.dat_val dat_pol,'' dateavanant,a.id_avis,a.dat_avis,p.dat_eff,p.dat_ech,s.nom_sous,s.pnom_sous,
                                        a.mtt_avis,a.cod_ref,a.cod_av,a.base_pn_commission,a.mtt_cpl,a.mtt_dt
                                from avis_recette  a, policew as p ,souscripteurw as s,utilisateurs u
                                where a.sens_avis=0 and a.type_avis=0 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2' and u.id_par='$user_selected'
                                        and (DATE_FORMAT(p.dat_val,'%Y-%m-%d') between '$date1' and '$date2' and a.cod_ref=p.cod_pol ) and p.cod_sous=s.cod_sous and s.id_user=u.id_user");
            $rqt->execute();
            $rqtav=$bdd->prepare("select  v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,a.dat_avis,v.dat_eff,v.dat_ech,
                                        s.nom_sous,s.pnom_sous,a.mtt_avis,a.cod_ref,a.cod_av,a.base_pn_commission,a.mtt_cpl,a.mtt_dt,v.lib_mpay
                                  from avis_recette  a, policew as p ,souscripteurw as s,avenantw as v,utilisateurs u
                                  where a.sens_avis=0 and a.type_avis=1 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2' and u.id_par='$user_selected'
                                        and (DATE_FORMAT(v.dat_val,'%Y-%m-%d') between '$date1' and '$date2' and a.cod_ref=v.cod_pol and a.cod_av=v.cod_av ) and p.cod_sous=s.cod_sous
                                        and s.id_user=u.id_user and p.cod_pol=v.cod_pol");
            $rqtav->execute();
            ...
            $rqtavneg=$bdd->prepare("select  v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,a.dat_avis,v.dat_eff,v.dat_ech,
                                                s.nom_sous,s.pnom_sous,a.mtt_avis,a.cod_ref,a.cod_av,a.base_pn_commission,a.mtt_cpl,a.mtt_dt,v.lib_mpay
                                    from avis_recette  a, policew as p ,souscripteurw as s,avenantw as v,utilisateurs u
                                    where a.sens_avis=1 and a.type_avis=1 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2' and u.id_par='$user_selected'
                                            and (DATE_FORMAT(v.dat_val,'%Y-%m-%d') between '$date1' and '$date2' and a.cod_ref=v.cod_pol and a.cod_av=v.cod_av ) and p.cod_sous=s.cod_sous
                                            and s.id_user=u.id_user and p.cod_pol=v.cod_pol");
            $rqtavneg->execute();
            ...
            $rqtavneg=$bdd->prepare("select  v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,a.dat_avis,v.dat_eff,v.dat_ech,
                                              s.nom_sous,s.pnom_sous,a.mtt_avis,a.cod_ref,a.cod_av,a.base_pn_commission,a.mtt_cpl,a.mtt_dt,v.lib_mpay
                                     from avis_recette  a, policew as p ,souscripteurw as s,avenantw as v,utilisateurs u
                                     where a.sens_avis=1 and a.type_avis=1 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2' and u.agence='$cas_par_agence'
                                              and (DATE_FORMAT(v.dat_val,'%Y-%m-%d') between '$date1' and '$date2' and a.cod_ref=v.cod_pol and a.cod_av=v.cod_av ) and p.cod_sous=s.cod_sous
                                              and s.id_user=u.id_user and p.cod_pol=v.cod_pol");
            $rqtavneg->execute();
        }
        ...
        $pdf->Cell(57, 7, ''.$lib_prod, '1', '0', 'L','1');
        $pdf->Cell(10, 7, 'S', '1', '0', 'C','1');
        $pdf->Cell(65, 7, '' . number_format($total_pn, 2, ',', ' ') . '', '1', '0', 'R','1');
        $pdf->Cell(55, 7, '' . number_format($total_cpl, 2, ',', ' ') . '', '1', '0', 'R','1');
        $pdf->Cell(70, 7, '' . number_format($total_commerciale, 2, ',', ' ') . '', '1', '0', 'R','1');
        $pdf->Cell(55, 7, '' . number_format($total_droit_timbre, 2, ',', ' ') . '', '1', '0', 'R','1');
        $pdf->Cell(93, 7, '' . number_format($ttc, 2, ',', ' ') . '', '1', '0', 'R','1');
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 10);
        $totalg_pn += $total_pn;
