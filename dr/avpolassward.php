...
$rqtc=$bdd->prepare("SELECT d.`cod_av`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.`lib_mpay`,s.`nom_sous`,s.`pnom_sous`,d.mtt_reg,d.mtt_solde  FROM `policew` as p,`avenantw` as d,`souscripteurw` as s WHERE s.`cod_sous`=p.`cod_sous` AND d.`cod_prod`='5' AND d.`cod_pol`=p.`cod_pol` AND d.`cod_pol`='$code' AND s.`nom_sous` LIKE '%$rech%' ORDER BY d.`cod_av` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
...
//Calcule du nombre de page 
$rqtc=$bdd->prepare("SELECT d.`cod_av`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.`lib_mpay`,s.`nom_sous`,s.`pnom_sous`,d.mtt_reg,d.mtt_solde  FROM `policew` as p,`avenantw` as d,`souscripteurw` as s WHERE s.`cod_sous`=p.`cod_sous` AND d.`cod_prod`='5' AND d.`cod_pol`=p.`cod_pol` AND d.`cod_pol`='$code' ORDER BY d.`cod_av` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_av`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.`lib_mpay`,s.`nom_sous`,s.`pnom_sous`,d.mtt_reg,d.mtt_solde  FROM `policew` as p,`avenantw` as d,`souscripteurw` as s WHERE s.`cod_sous`=p.`cod_sous` AND d.`cod_prod`='5' AND d.`cod_pol`=p.`cod_pol` AND d.`cod_pol`='$code' ORDER BY d.`cod_av` DESC LIMIT $part ,7");
$rqt->execute();
$nb = $rqt->execute();
}
?>  
  ...
  <div class="widget-box">
            <ul class="quick-actions">
			  <li class="bg_lo"> <a onClick="dMenu1('macc','../ddash.php')"> <i class="icon-home"></i>Acceuil </a> </li>
				<li class="bg_lv"> <a onClick="dMenu1('prod','polassward.php')"> <i class="icon-backward"></i>Precedent</a></li>
...
<script language="JavaScript">
	function frechapwar(){
        ...
        $("#content").load('dr/avpolassward.php?rech='+rech);
	}
	function fpageapwar(page,nbpage,code){
		if(page >=0){
			if(page == nbpage){
				swal("Information !","Vous ete a la derniere page!","info");
			}else{$("#content").load('dr/avpolassward.php?page='+page+'&code='+code);}
