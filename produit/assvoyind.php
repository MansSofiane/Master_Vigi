...
`devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  AND $condition AND d.`cod_formul`='2' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence') AND $condition AND d.`cod_formul`='2' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();

}else {
//Calcule du nombre de page
    $rqtc = $bdd->prepare("SELECT d.`cod_dev`,d.`cod_per`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM
`devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  AND d.`cod_formul`='2' ORDER BY
d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage = ceil($nbe / 7);
        ...
        <table class="table table-bordered data-table">
            ...
            while ($row_res=$rqt->fetch()){  ?>
                    ...
                    <td>&nbsp;
                        ...
                        <a onClick="vdevoy('<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['dat_eff'];?>','assvoyind.php')" title="Valider"><i CLASS="icon-ok icon-2x" style="color:green"/></a>&nbsp;&nbsp;&nbsp;

                        <a onClick="ddev('prod','assvoyind.php','<?php echo $row_res['cod_dev'];?>')" title="Supprimer"><i CLASS="icon-trash icon-2x" style="color:red"/></a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
...
*/
    ...
    function vdevoy (codedev,date,page){
        ...
        if(compdat2(date)){
            ...
            swal({
                title: "Validation !",
                text: "Comfirmez la Souscription ",
