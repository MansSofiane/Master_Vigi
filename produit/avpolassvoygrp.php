                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <?php if ($row_res['mtt_solde']<>0){ ?>
                            <a onClick="reglement('<?php echo $row_res['cod_av'];?>','1','<?php echo $code;//code police?>','avpolassvoygrp.php')" title="Regler"><i CLASS="icon-money icon-2x" style="color:green"  /></a>&nbsp;&nbsp;&nbsp;
                        <?php }?>
...
<script language="JavaScript">
    ...
    */
    function frechapconso(){
        ...
        $("#content").load('produit/avpolassvoygrp.php?rech='+rech);
    }
    function fpageapconso(page,nbpage,code){
        if(page >=0){
            if(page == nbpage){
                swal("Information !","Vous ete a la derniere page!","info")
            }else{$("#content").load('produit/avpolassvoygrp.php?page='+page+'&code='+code);}
