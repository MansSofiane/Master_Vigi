...
$rqtg = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,d.`pn`,d.`pt`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod` ,
								d.`mtt_reg`, d.`mtt_solde`,s.`cod_sous`,s.`nom_sous`, s.`pnom_sous`, s.`adr_sous`,m.`lib_mpay`,u.`agence`,y.`lib_pays`,a.lib_agence as apporteur
						FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u, `pays` as y, `agence` as a
						WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays` AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user`
								AND u.`agence`='$user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND d.cod_agence=a.cod_agence
");
$rqtg->execute();

//Requete extraction des avenants

$rqtv = $bdd->prepare("SELECT d.`dat_val`,d.`pn`,d.`pn`,d.`pt`,d.`lib_mpay`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod`,
								d.`mtt_reg`, d.`mtt_solde`,s.`cod_sous`,s.`nom_sous`, s.`pnom_sous`, s.`adr_sous`,z.sequence as seq2, z.dat_val as datev,u.`agence`,y.`lib_pays`,a.`lib_agence` as apporteur
					  FROM `avenantw` as d,`policew` as z, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s,`utilisateurs` as u, `pays` as y,`agence` as a
					  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_pol`=z.`cod_pol` AND z.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays` AND s.`id_user`=u.`id_user`
					   			AND u.`agence`='$user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND z.cod_agence=a.cod_agence
");
...
$rqtg = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,d.`pn`,d.`pt`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod` ,
						...
						FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u, `pays` as y, `agence` as a
						WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays` AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user`
								AND u.`id_par`='$id_user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND d.cod_agence=a.cod_agence
");
$rqtg->execute();
...
$rqtv = $bdd->prepare("SELECT d.`dat_val`,d.`pn`,d.`pn`,d.`pt`,d.`lib_mpay`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod`,
						...
						FROM `avenantw` as d,`policew` as z, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s,`utilisateurs` as u, `pays` as y,`agence` as a
						WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_pol`=z.`cod_pol` AND z.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays` AND s.`id_user`=u.`id_user`
								AND u.`id_par`='$id_user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND z.cod_agence=a.cod_agence
");
...
// Bordure de table
$objPHPExcel->getActiveSheet()->getStyle('A9:AA9')->applyFromArray(
		array(
			'font'    => array(
				'bold'      => true
			),
...
$objPHPExcel->getActiveSheet()->setCellValue('T9', 'Prime Totale');
$objPHPExcel->getActiveSheet()->setCellValue('U9', 'Montant Regle');
$objPHPExcel->getActiveSheet()->setCellValue('V9', 'Solde');
$objPHPExcel->getActiveSheet()->setCellValue('W9', 'Nom /Rs Souscripteur');
$objPHPExcel->getActiveSheet()->setCellValue('X9', 'Prenom Souscripteur');
$objPHPExcel->getActiveSheet()->setCellValue('Y9', 'Adresse Souscripteur');
$objPHPExcel->getActiveSheet()->setCellValue('Z9', 'Nb personne');
$objPHPExcel->getActiveSheet()->setCellValue('AA9', 'Apporteur');
...
$objPHPExcel->getActiveSheet()->getStyle('A9:AA9')->applyFromArray($styleThinBlackBorderOutline);
...
$objPHPExcel->getActiveSheet()->getStyle('Y')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE);
$objPHPExcel->getActiveSheet()->getStyle('Z')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE);
$objPHPExcel->getActiveSheet()->getStyle('AA')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE);
...
$cpt='0';
$j=9;
$tpn=0;$tcp=0;$tdt=0;$tpt=0;$tmtt_reg=0; $tmtt_solde=0;
...
$objPHPExcel->getActiveSheet()->setCellValue('D'.$j, $user);
...
$objPHPExcel->getActiveSheet()->setCellValue('T'.$j, $row_g['pt']);$tpt=$tpt+$row_g['pt'];
$objPHPExcel->getActiveSheet()->setCellValue('U'.$j, $row_g['mtt_reg']);$tmtt_reg=$tmtt_reg+$row_g['mtt_reg'];
$objPHPExcel->getActiveSheet()->setCellValue('V'.$j, $row_g['mtt_solde']);$tmtt_solde=$tmtt_solde+$row_g['mtt_solde'];
$objPHPExcel->getActiveSheet()->setCellValue('W'.$j, $row_g['nom_sous']);
$objPHPExcel->getActiveSheet()->setCellValue('X'.$j, $row_g['pnom_sous']);
$objPHPExcel->getActiveSheet()->setCellValue('Y'.$j, $row_g['adr_sous']);
$objPHPExcel->getActiveSheet()->setCellValue('Z'.$j, nbassu($row_g['cod_sous']));
$objPHPExcel->getActiveSheet()->setCellValue('AA'.$j, $row_g['apporteur']);
...
$objPHPExcel->getActiveSheet()->setCellValue('T'.$j, $row_v['pt']);$tpt=$tpt+$row_v['pt'];
$objPHPExcel->getActiveSheet()->setCellValue('U'.$j, $row_v['mtt_reg']);$tmtt_reg=$tmtt_reg+$row_v['mtt_reg'];
$objPHPExcel->getActiveSheet()->setCellValue('V'.$j, $row_v['mtt_solde']);$tmtt_solde=$tmtt_solde+$row_v['mtt_solde'];
$objPHPExcel->getActiveSheet()->setCellValue('W'.$j, $row_v['nom_sous']);
$objPHPExcel->getActiveSheet()->setCellValue('X'.$j, $row_v['pnom_sous']);
$objPHPExcel->getActiveSheet()->setCellValue('Y'.$j, $row_v['adr_sous']);
$objPHPExcel->getActiveSheet()->setCellValue('Z'.$j, nbassu($row_v['cod_sous']));
$objPHPExcel->getActiveSheet()->setCellValue('AA'.$j, $row_v['apporteur']);
...
$objPHPExcel->getActiveSheet()->getStyle('A9:AA'.$j)->applyFromArray($styleThinBlackBorderOutline);
// la derniere ligne des totaux 

$i=$j+1;

//**************
for($lettre2='Q'; $lettre2!='W';$lettre2++){
...
$objPHPExcel->getActiveSheet()->setCellValue('T'.$i, $tpt);
$objPHPExcel->getActiveSheet()->setCellValue('U'.$i, $tmtt_reg);
$objPHPExcel->getActiveSheet()->setCellValue('V'.$i, $tmtt_solde);
$objPHPExcel->getActiveSheet()->getStyle('Q'.$i.':V'.$i)->applyFromArray($styleThinBlackBorderOutline);
...
// Requete des assuré
if($user!=0){
$rqtass = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,p.`code_prod`,p.`lib_prod` ,s.`cod_sous`,s.`nom_sous`,s.`pnom_sous`,s.`adr_sous`,s.`dnais_sous`,s.`rp_sous`,s.`nb_assu`,u.`agence` FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `utilisateurs` as u  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND s.`id_user`=u.`id_user` AND u.`agence`='$user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef'");
$rqtass->execute();
}else{
$rqtass = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,p.`code_prod`,p.`lib_prod` ,s.`cod_sous`,s.`nom_sous`,s.`pnom_sous`,s.`adr_sous`,s.`dnais_sous`,s.`rp_sous`,s.`nb_assu`,u.`agence` FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `utilisateurs` as u  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND s.`id_user`=u.`id_user` AND u.`id_par`='$id_user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef'");
$rqtass->execute();
