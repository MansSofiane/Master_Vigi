...
// Instanciation de la classe derivee
    ...
    $pdf->Cell(55,5,'Police N°','1','0','C','1');$pdf->Cell(55,5,'Avenant N°','1','0','C','1');$pdf->Cell(60,5,'Nom&Prénom-R.Sociale','1','0','C','1');
    $pdf->Cell(20,5,'Emmision','1','0','C','1');$pdf->Cell(20,5,'Effet','1','0','C','1');$pdf->Cell(20,5,'Echéance','1','0','C','1');

    $pdf->Cell(10,5,'Sens','1','0','C','1');$pdf->Cell(40,5,'P.Nette','1','0','C','1');$pdf->Cell(25,5,'C.Police','1','0','C','1');$pdf->Cell(40,5,'P.Commer','1','0','C','1');$pdf->Cell(20,5,'D.Timbre','1','0','C','1');$pdf->Cell(40,5,'P.Total','1','0','C','1');
    ...
    while ($rwg=$rqtag->fetch())
    {
        $agence= $rwg['agence'];
            ...
            $rqt=$bdd->prepare("select  case when  a.sens_avis=0 then 'E' else 'D' end as etatcaisse,p.cod_pol police,'0' as avenant,p.cod_prod,p.sequence sequence_police,'' sequence_av,p.dat_val dat_pol,'' dateavanant,a.id_avis,a.dat_avis,p.dat_eff,p.dat_ech,
                                ...
                                where a.sens_avis=0 and a.type_avis=0 and p.cod_prod='$prod' and u.agence='$agence' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d')  between '$date1' and '$date2'
                                      and (a.cod_ref=p.cod_pol ) and p.cod_sous=s.cod_sous and s.id_user=u.id_user");
            $rqt->execute();
            $rqtav=$bdd->prepare("select  case when  a.sens_avis=0 then 'E' else 'D' end as etatcaisse,v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,
                                  ...
                                  where a.sens_avis=0 and a.type_avis=1 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2' and ( a.cod_ref=v.cod_pol and a.cod_av=v.cod_av )
                                          and p.cod_sous=s.cod_sous and s.id_user=u.id_user and u.agence='$agence' and p.cod_pol=v.cod_pol");
            $rqtav->execute();
            ...
            $rqtavneg=$bdd->prepare("select  case when  a.sens_avis=0 then 'E' else 'D' end as etatcaisse,v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,
                                    ...
                                    where a.sens_avis=1 and a.type_avis=1 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2'
                                              and ( a.cod_ref=v.cod_pol and a.cod_av=v.cod_av ) and p.cod_sous=s.cod_sous and s.id_user=u.id_user and u.agence='$agence' and p.cod_pol=v.cod_pol");
            $rqtavneg->execute();
        ...
        while ($row_g=$rqt->fetch()) {
            ...
            $pdf->Cell(40, 5, ''.number_format($row_g['base_pn_commission'], 2,',',' ').'','1','0','R');$total_pn+=$row_g['base_pn_commission'];$total_pn_i+=$row_g['base_pn_commission'];
            $pdf->Cell(25, 5, ''.number_format($row_g['mtt_cpl'], 2,',',' ').'','1','0','R');$total_cpl+=$row_g['mtt_cpl'];$total_cpl_i+=$row_g['mtt_cpl'];
            $pdf->Cell(40, 5, ''.number_format($row_g['base_pn_commission']+$row_g['mtt_cpl'], 2,',',' ').'','1','0','R');$total_commerciale+=$row_g['base_pn_commission']+$row_g['mtt_cpl'];$total_commerciale_i+=$row_g['base_pn_commission']+$row_g['mtt_cpl'];
            $pdf->Cell(20, 5, ''.number_format($row_g['mtt_dt'], 2,',',' ').'','1','0','R');$total_droit_timbre+=$row_g['mtt_dt'];$total_droit_timbre_i+=$row_g['mtt_dt'];
            $pdf->Cell(40, 5, ''.number_format($row_g['mtt_avis'], 2,',',' ').'','1','0','R');$ttc+=$row_g['mtt_avis'];$ttc_i+=$row_g['mtt_avis'];
            $pdf->Ln();
       ...
       // $pdf->SetFillColor(173,253,207);
        ...
        $pdf->Cell(240, 10, 'Total Encaissement et Recouverement ' .$agence. '', '1', '0', 'L');
        ...
        while ($row_g=$rqtavneg->fetch()) {
            ...
            $pdf->Cell(20, 5, ''.date("d/m/Y", strtotime($row_g['dat_ech'])), '1', '0', 'C');
            $pdf->Cell(10, 5, $row_g['etatcaisse'], '1', '0', 'L');
            $pdf->Cell(40, 5, ''.number_format($row_g['base_pn_commission'], 2,',',' ').'','1','0','R');$total_pn+=$row_g['base_pn_commission'];$total_pn_i+=$row_g['base_pn_commission'];$total_pn_ineg+=$row_g['base_pn_commission'];
            $pdf->Cell(25, 5, ''.number_format($row_g['mtt_cpl'], 2,',',' ').'','1','0','R');$total_cpl+=$row_g['mtt_cpl'];$total_cpl_i+=$row_g['mtt_cpl'];$total_cpl_ineg+=$row_g['mtt_cpl'];
            $pdf->Cell(40, 5, ''.number_format($row_g['base_pn_commission']+$row_g['mtt_cpl'], 2,',',' ').'','1','0','R');$total_commerciale+=$row_g['base_pn_commission']+$row_g['mtt_cpl'];$total_commerciale_i+=$row_g['base_pn_commission']+$row_g['mtt_cpl'];$total_commerciale_ineg+=$row_g['base_pn_commission']+$row_g['mtt_cpl'];
            $pdf->Cell(20, 5, ''.number_format($row_g['mtt_dt'], 2,',',' ').'','1','0','R');$total_droit_timbre+=$row_g['mtt_dt'];$total_droit_timbre_i+=$row_g['mtt_dt'];$total_droit_timbre_ineg+=$row_g['mtt_dt'];
            $pdf->Cell(40, 5, ''.number_format($row_g['mtt_avis'], 2,',',' ').'','1','0','R');$ttc+=$row_g['mtt_avis'];$ttc_i+=$row_g['mtt_avis'];$ttc_ineg+=$row_g['mtt_avis'];
            $pdf->Ln();
       ...
       // $pdf->SetFillColor(249,215,158);
        ...
        $pdf->Cell(240, 10, 'Total Decaissement ' .$agence. '', '1', '0', 'L');
        ...
        if ($total_pn_i>0)
           ...
           $pdf->SetFillColor(249,215,158);
        $pdf->Cell(240, 10, 'Solde caisse agence ' .$agence. '', '1', '0', 'L','1');
