...
// Instanciation de la classe derivee
    ...
    $rqtuu = $bdd->prepare("SELECT * FROM `utilisateurs` as a where a.agence='$agence_selected'");
    $rqtuu->execute();
    $condition_agence = '';
        ...
        $code_prod=$rp['code_prod'];
            ...
            $rqtav=$bdd->prepare("select  v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,
                                  ...
                                  where a.sens_avis=0 and a.type_avis=1 and p.cod_prod='$prod' and DATE_FORMAT(a.dat_avis,'%Y-%m-%d') between '$date1' and '$date2'
                                          ...
                                          and s.id_user=u.id_user $condition_agence and p.cod_pol=v.cod_pol");
            $rqtav->execute();


        $rqtavneg=$bdd->prepare("select  v.cod_pol police,v.cod_av as avenant,v.cod_prod,p.sequence sequence_police,v.sequence sequence_av,p.dat_val dat_pol ,v.dat_val dateavanant,a.id_avis,a.dat_avis,
