...
?>
  ...
  <div id="content-header">
    <div id="breadcrumb"> <a class="tip-bottom"><i class="icon-signal"></i> Etats</a><a class="current">Production-Excel</a></div>
  </div>
  ...
  <div class="widget-box">
            ...
            <ul class="quick-actions">
			  ...
			  <li class="bg_lv"> <a onClick="prod('100','<?php echo $agence; ?>')"> <i class="icon-bar-chart"></i>production globale</a></li>
			</ul>
</div>
...
function prod(code,user) {
	...
	if (verifdatestat(date1) && verifdatestat(date2)) {
		...
		window.open('excel/Excel-P/' + user + '/' + d1 + '/' + d2+ '/' + code,"Generation Excel","menubar=no, status=no, scrollbars=yes, menubar=no, width=600, height=230");
