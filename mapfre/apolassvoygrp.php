...
as d,`souscripteurw` as s, utilisateurs as u WHERE s.`cod_sous`=d.`cod_sous`  AND d.`cod_prod`='1'  and d.cod_formul='5' AND s.id_user=u.id_user AND s.`nom_sous` LIKE '%$rech%' ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
as d,`souscripteurw` as s, utilisateurs as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='5' AND s.id_user=u.id_user AND s.`nom_sous` LIKE '%$rech%' ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
...
<div class="widget-box">
    <ul class="quick-actions">
        <li class="bg_lo"> <a onClick="mMenu1('macc','../mdash.php')"> <i class="icon-home"></i>Acceuil </a> </li>
        <li class="bg_lv"> <a onClick="mMenu1('prod','assvoy.php')"> <i class="icon-backward"></i>Precedent</a></li>
    </ul>
</div>
<div class="widget-box">
    <div class="widget-title">
        <div><input type="text" id="nsousvgrp" onchange="afrechvgrps()" class="span4" placeholder="Rechercher par Nom-Souscripteur..."/></div>
    </div>
    ...
    <div class="widget-title" align="center">
        ...
        <a href="javascript:;" title="Premiere page" onClick="afpagevgrps('0','<?php echo $nbpage; ?>')"><img  src="img/icons/fprec.png"/></a>
        <a href="javascript:;" title="Precedent" onClick="afpagevgrps('<?php echo $page-1; ?>','<?php echo $nbpage; ?>')"><img  src="img/icons/prec.png"/></a>
        <?php echo $page; ?>/<?php echo $nbpage; ?>
        <a href="javascript:;" title="Suivant" onClick="afpagevgrps('<?php echo $page+1; ?>','<?php echo $nbpage; ?>')"><img  src="img/icons/suiv.png"/></a>
        <a href="javascript:;" title="Derniere page" onClick="afpagevgrps('<?php echo $nbpage-1; ?>','<?php echo $nbpage; ?>')"><img  src="img/icons/fsuiv.png"/></a>
    </div>
</div>
<script language="JavaScript">
    function afrechvgrps(){
        var rech=document.getElementById("nsousvgrp").value;
        $("#content").load('mapfre/apolassvoygrp.php?rech='+rech);
    }
    function afpagevgrps(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('mapfre/apolassvoygrp.php?page='+page);}
