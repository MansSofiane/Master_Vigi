...
if($user!='0'){
	...
	//Requete par agence
			$rqtg = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,d.`pn`,d.`pt`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod`,
					   ...
					   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays`
					   			AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user` AND u.`agence`='$agencesel' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated'
					   			AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND d.cod_agence=a.cod_agence");
			$rqtg->execute();
	//Requete extraction des avenants

			$rqtv = $bdd->prepare("SELECT d.`dat_val`,d.`pn`,d.`pn`,d.`pt`,d.`lib_mpay`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,t.`mtt_dt`,c.`mtt_cpl`,
								p.`code_prod`,p.`lib_prod`, s.`cod_sous`,s.`nom_sous`, s.`pnom_sous`, s.`adr_sous`,z.sequence as seq2, z.dat_val as datev,u.`agence`,y.`lib_pays`,
								a.`lib_agence` as apporteur,d.`mtt_reg`, d.`mtt_solde`
					   ...
					   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_pol`=z.`cod_pol` AND z.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays`
					    		AND s.`id_user`=u.`id_user` AND u.`agence`='$agencesel' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated'
					    		AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND z.cod_agence=a.cod_agence");
		...
		//requete par DR
				...
				$rqtg = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,d.`pn`,d.`pt`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod`,
									   ...
									   FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u, `pays` as y, `agence` as a
									   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays`
												AND d.`mode`=m.`cod_mpay` AND s.`id_user`=u.`id_user` AND u.`id_par`='$user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated'
												AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND d.cod_agence=a.cod_agence");
				$rqtg->execute();
		...
		//Requete extraction des avenants
				...
				$rqtv = $bdd->prepare("SELECT d.`dat_val`,d.`pn`,d.`pn`,d.`pt`,d.`lib_mpay`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,t.`mtt_dt`,c.`mtt_cpl`,
									   ...
									   FROM `avenantw` as d,`policew` as z, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s,`utilisateurs` as u, `pays` as y,`agence` as a
									   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_pol`=z.`cod_pol` AND z.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays`
												AND s.`id_user`=u.`id_user` AND u.`id_par`='$user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef'
												AND z.cod_agence=a.cod_agence");

				$rqtv->execute();
		}

}else{
	$agencesel='Toutes les Agences';
//Requete generale

$rqtg = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,d.`pn`,d.`pt`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod`,
								d.`mtt_reg`, d.`mtt_solde`,s.`cod_sous`,s.`nom_sous`, s.`pnom_sous`, s.`adr_sous`,m.`lib_mpay`,u.`agence`,y.`lib_pays`,a.lib_agence as apporteur
					   FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `mpay` as m, `utilisateurs` as u, `pays` as y, `agence` as a
					   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays` AND d.`mode`=m.`cod_mpay`
					   AND s.`id_user`=u.`id_user` AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND d.cod_agence=a.cod_agence
");
$rqtg->execute();
...
$rqtv = $bdd->prepare("SELECT d.`dat_val`,d.`pn`,d.`pn`,d.`pt`,d.`lib_mpay`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,t.`mtt_dt`,c.`mtt_cpl`,
					   ...
					   FROM `avenantw` as d,`policew` as z, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s,`utilisateurs` as u, `pays` as y,`agence` as a
					   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_pol`=z.`cod_pol` AND z.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays`
					   			AND s.`id_user`=u.`id_user` AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef' AND z.cod_agence=a.cod_agence
");
...
//boucle pour le style
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$j, $row_g['agence']);
...
//Libelle du type de l'avenant
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$j, $row_v['agence']);
...
if($type=='user'){
//Agence
$rqtass = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,p.`code_prod`,p.`lib_prod` ,s.`cod_sous`,s.`nom_sous`,s.`pnom_sous`,s.`adr_sous`,s.`dnais_sous`,s.`rp_sous`,s.`nb_assu`,u.`agence`
						  FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `utilisateurs` as u
						  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND s.`id_user`=u.`id_user`
								AND u.`agence`='$agencesel' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef'");
$rqtass->execute();
}else{
//DR
$rqtass = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,p.`code_prod`,p.`lib_prod` ,s.`cod_sous`,s.`nom_sous`,s.`pnom_sous`,s.`adr_sous`,s.`dnais_sous`,s.`rp_sous`,s.`nb_assu`,u.`agence`
						  FROM `policew` as d, `dtimbre` as t , `cpolice` as c,`produit` as p,`souscripteurw` as s, `utilisateurs` as u
						  WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND s.`id_user`=u.`id_user` AND u.`id_par`='$user'
						  		AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef'");
$rqtass->execute();
