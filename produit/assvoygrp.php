...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  AND $condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='5' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='5' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();

}else {
//Calcule du nombre de page
    $rqtc = $bdd->prepare("SELECT d.`cod_dev`,d.`cod_per`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,p.`lib_per` FROM
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='5' ORDER BY
d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage = ceil($nbe / 7);
        ...
        <table class="table table-bordered data-table">
            ...
            while ($row_res=$rqt->fetch()){  ?>
                    ...
                    <td>&nbsp;
                       ...
                       <!-- <a onClick="mdevoy('<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['dat_eff'];?>','assvoygrp.php')" title="Modifier"><img  src="img/icons/mpol.png"/></a>&nbsp;&nbsp;&nbsp;-->
                       <a onClick="Afficher('<?php echo crypte($row_res['cod_dev']);?>','assvoygrp.php')" title="Assures"><i CLASS="icon-group icon-2x" style="color:grey"/></a>&nbsp;&nbsp;&nbsp;
  ...
  /*  function frechdade(){
    ...
    function vdevoy (codedev,date,page) {
        ...
        if (compdat2(date)) {
            ...
            swal("Oops..", "Date d'effet du devis d\351pass\351e !", "error");
        }
    ...
    function mdevoy (codedev,date,page){
        ...
        if(compdat2(date)){
            ...
            swal({
                title: "Chargement !",
                text: "Voulez-vous recharger la liste des assures ?",
                showCancelButton: true,
