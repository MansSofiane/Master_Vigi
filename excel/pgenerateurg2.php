...
$rqtg = $bdd->prepare("SELECT d.`dat_val`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`cap1`,d.`cap2`,d.`cap3`,d.`pn`,d.`p1`,d.`p2`,d.`p3`,d.`pn`,d.`pt`,t.`mtt_dt`,c.`mtt_cpl`,p.`code_prod`,p.`lib_prod` ,s.`nom_sous`,
					   ...
					   WHERE d.`cod_dt`=t.`cod_dt` AND d.`cod_cpl`=c.`cod_cpl` AND d.`cod_prod`=p.`cod_prod` AND d.`cod_sous`=s.`cod_sous` AND d.`cod_pays`=y.`cod_pays` AND d.`mode`=m.`cod_mpay`
					   		AND s.`id_user`=u.`id_user` AND u.`id_user`='$user' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') >='$dated' AND DATE_FORMAT(d.`dat_val`,'%Y-%m-%d') <='$datef'");
$rqtg->execute();
...
$objPHPExcel->getActiveSheet()->setCellValue('Y9', 'Montant Regle');
$objPHPExcel->getActiveSheet()->setCellValue('Z9', 'Solde');
...
$objPHPExcel->getActiveSheet()->getStyle('A9:Z9')->applyFromArray($styleThinBlackBorderOutline);
...
$objPHPExcel->getActiveSheet()->setCellValue('Y'.$j, $row_g['mtt_reg']);$tmtt_reg=$tmtt_reg+$row_g['mtt_reg'];
$objPHPExcel->getActiveSheet()->setCellValue('Z'.$j, $row_g['mtt_solde']);$tmtt_solde=$tmtt_solde+$row_g['mtt_solde'];
...
$objPHPExcel->getActiveSheet()->getStyle('A'.$j.':Z'.$j)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A'.$j.':Z'.$j)->getFill()->getStartColor()->setARGB('FAFAD2');
$objPHPExcel->getActiveSheet()->setCellValue('A'.$j, $cpt);
$objPHPExcel->getActiveSheet()->setCellValue('B'.$j, $agence);
...
$objPHPExcel->getActiveSheet()->setCellValue('Y'.$j, $row_v['mtt_reg']);$tmtt_reg=$tmtt_reg+$row_v['mtt_reg'];
$objPHPExcel->getActiveSheet()->setCellValue('Z'.$j, $row_v['mtt_solde']);$tmtt_solde=$tmtt_solde+$row_v['mtt_solde'];
...
$objPHPExcel->getActiveSheet()->getStyle('A9:Z'.$j)->applyFromArray($styleThinBlackBorderOutline);
// la derniere ligne des totaux 
$i=$j+1;

//**************
for($lettre2='P'; $lettre2!='AA';$lettre2++){
...
$objPHPExcel->getActiveSheet()->setCellValue('Y'.$i, $tmtt_reg);
$objPHPExcel->getActiveSheet()->setCellValue('Z'.$i, $tmtt_solde);
$objPHPExcel->getActiveSheet()->getStyle('Q'.$i.':Z'.$i)->applyFromArray($styleThinBlackBorderOutline);
// Nom de la Feuille 
