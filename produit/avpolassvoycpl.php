...
//Calcule du nombre de page
    $rqtc=$bdd->prepare("SELECT d.`cod_av`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.`lib_mpay`,s.`nom_sous`,s.`pnom_sous`,s.rp_sous,d.mtt_reg,d.mtt_solde FROM `policew` as p,`avenantw` as d,`souscripteurw` as s WHERE s.`cod_sous`=p.`cod_sous` AND d.`cod_prod`='1' AND d.`cod_pol`=p.`cod_pol` AND d.`cod_pol`='$code' AND s.`nom_sous` LIKE '%$rech%' ORDER BY d.`cod_av` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
//Calcule du nombre de page
    $rqtc=$bdd->prepare("SELECT d.`cod_av`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.`lib_mpay`,s.`nom_sous`,s.`pnom_sous`,s.rp_sous,d.mtt_reg,d.mtt_solde FROM `policew` as p,`avenantw` as d,`souscripteurw` as s WHERE s.`cod_sous`=p.`cod_sous` AND d.`cod_prod`='1' AND d.`cod_pol`=p.`cod_pol` AND d.`cod_pol`='$code' ORDER BY d.`cod_av` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
                   ...
                   <?php
                    ...
                    <td>&nbsp;
                        ...
                        <?php if ($row_res['mtt_solde']<>0){ ?>
                            <a onClick="reglement('<?php echo $row_res['cod_av'];?>','1','<?php echo $code;//code police?>','avpolassvoycpl.php')" title="Regler"><i CLASS="icon-money icon-2x" style="color:green"  /></a>&nbsp;&nbsp;&nbsp;
                        <?php }?>
                    </td>
                </tr>
            <?php } ?>
...
<script language="JavaScript">
    function frechapconso(){
        ...
        $("#content").load('produit/avpolassvoycpl.php?rech='+rech);
    }
    function fpageapconso(page,nbpage,code){
        if(page >=0){
            if(page == nbpage){
                swal("Information !","Vous ete a la derniere page!","info")
            }else{$("#content").load('produit/avpolassvoycpl.php?page='+page+'&code='+code);}
