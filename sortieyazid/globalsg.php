...
// Instanciation de la classe derivee
    ...
    if ($agence == '0') {
        $pdf->Cell(210, 10, 'Etat global: ', '1', '0', 'L');
        $pdf->Cell(170, 10, 'Produit: Tous les produits ', '1', '0', 'L');
       // $pdf->Cell(130, 10, 'Code produit: ', '1', '1', 'C');
    }else
    {
        if($typ_usr=='dr') {
            $pdf->Cell(210, 10, 'Direction Regionale:'.$cas_par_agence, '1', '0', 'L');
            $pdf->Cell(170, 10, 'Produit: Tous les produits ', '1', '0', 'L');
            //$pdf->Cell(130, 10, 'Code produit: ', '1', '1', 'C');
        }else
        {
    ...
    $pdf->Cell(40,10,'Produits','1','0','C');$pdf->Cell(50,10,'Production','1','0','C'); $pdf->Cell(20,10,'Nombre','1','0','C');$pdf->Cell(40,10,'P.Nette','1','0','C');$pdf->Cell(30,10,'Accessoire','1','0','C');$pdf->Cell(40,10,'P.Commer','1','0','C');$pdf->Cell(20,10,'D.T','1','0','C');$pdf->Cell(50,10,'P.Total','1','0','C');$pdf->Cell(50,10,'Encaissement/Décaissement','1','0','C');$pdf->Cell(40,10,'Solde (*)','1','0','C');
    $pdf->Ln();
 ...
 //Boucle police
    ...
    while ($rowp= $rqt_prod->fetch()) {
        ...
        if ($agence == '0') {
            ...
            $rqtpos = $bdd->prepare("select count(cod_doc) as nb, sum(prime_nette) as prime_nette, sum(cout_police) as cout_police,sum(prime_com)as prime_com, sum(droit_timbre) as droit_timbre, sum(prime_totale) as prime_totale,sum(reglement) as Reglement,sum(solde) as Solde,table1.agence as agence,table1.cod_prod,table1.produits as produits
from (
select p.sequence as cod_doc, p.dat_val as dat_val,p.ndat_eff as ndat_eff,p.ndat_ech as ndat_ech, p.pn as prime_nette, c.mtt_cpl as cout_police,p.pn+c.mtt_cpl as prime_com, d.mtt_dt as droit_timbre, p.pt as prime_totale,p.mtt_reg as reglement,p.mtt_solde as solde,u.agence as agence,pr.cod_prod as cod_prod,pr.lib_prod as produits
...
select v.sequence as cod_doc,p.dat_val as dat_val,p.ndat_eff as ndat_eff,p.ndat_ech as ndat_ech,v.pn as prime_nette, c.mtt_cpl as cout_police,v.pn+c.mtt_cpl as prime_com, d.mtt_dt as droit_timbre, v.pt as prime_totale,v.mtt_reg as reglement,v.mtt_solde as solde,u.agence as agence,pr.cod_prod as cod_prod,pr.lib_prod as produits
...
group by table1.cod_prod");
            ...
            $rqtrist = $bdd->prepare("select count(cod_doc) as nb, sum(prime_nette) as prime_nette, sum(cout_police) as cout_police,sum(prime_com)as prime_com, sum(droit_timbre) as droit_timbre, sum(prime_totale) as prime_totale,sum(reglement) as Reglement,sum(solde) as Solde,table1.agence as agence,table1.cod_prod,table1.produits as produits
from (
...
group by table1.cod_prod");
            ...
            $rqt_s_rist = $bdd->prepare("select count(cod_doc) as nb, sum(prime_nette) as prime_nette, sum(cout_police) as cout_police,sum(prime_com)as prime_com, sum(droit_timbre) as droit_timbre, sum(prime_totale) as prime_totale,sum(reglement) as Reglement,sum(solde) as Solde,table1.agence as agence,table1.cod_prod,table1.produits as produits
from (
...
group by table1.cod_prod");
        ...
        while($rows_p_pi=$rqtpos->fetch())
        {
            ...
            $solde_total_positif_pi=$rows_p_pi['Solde'];
        }
        ...
        while($rows_ar_pi=$rqtrist->fetch())
        {
            ...
            $solde_total_ar_pi=$rows_ar_pi['Solde'];
        }
        ...
        while($rows_sr_pi=$rqt_s_rist->fetch())
        {
            ...
            $solde_total_sr_pi=$rows_sr_pi['Solde'];
        }
        ...
        $pdf->Cell(40,6,''.number_format($prime_nette_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,6,''.number_format($Accessoire_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($prime_commerciale_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,6,''.number_format($dt_positifs_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($prime_total_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($reglement_total_positif_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($solde_total_positif_pi, 2,',',' ').'','1','0','R');$pdf->Ln(7);
        $X=$pdf->GetX();
        $pdf->SetX($X+40);
        ...
        $pdf->Cell(40,6,''.number_format($prime_nette_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,6,''.number_format($Accessoire_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($prime_commerciale_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,6,''.number_format($dt_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($prime_total_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($reglement_total_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($solde_total_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Ln(6);
        $X=$pdf->GetX();
        $pdf->SetX($X+40);
        // $pdf->Cell(50,10,''.$produits,'1','0','C');
        ...
        $pdf->Cell(40,6,''.number_format($prime_nette_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,6,''.number_format($Accessoire_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($prime_commerciale_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,6,''.number_format($dt_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($prime_total_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($reglement_total_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($solde_total_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Ln(6);
        $X=$pdf->GetX();
        $pdf->SetX($X+40);
        $pdf->SetFont('Arial','B',12);
        ...
        $pdf->Cell(20,6,'','1','0','C');
        $pdf->Cell(40,6,''.number_format($prime_nette_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,6,''.number_format($Accessoire_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($prime_commerciale_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,6,''.number_format($dt_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($prime_total_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,6,''.number_format($reglement_total_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,6,''.number_format($solde_total_pi, 2,',',' ').'','1','0','R');
        $pdf->Ln(6);
    ...
    $pdf->Cell(40,10,''.number_format($prime_nette, 2,',',' ').'','1','0','R');
    $pdf->Cell(30,10,''.number_format($Accessoire, 2,',',' ').'','1','0','R');
    $pdf->Cell(40,10,''.number_format($prime_commerciale, 2,',',' ').'','1','0','R');
    $pdf->Cell(20,10,''.number_format($dt, 2,',',' ').'','1','0','R');
    $pdf->Cell(50,10,''.number_format($prime_total, 2,',',' ').'','1','0','R');
    $pdf->Cell(50,10,''.number_format($reglement_total, 2,',',' ').'','1','0','R');
    $pdf->Cell(40,10,''.number_format($solde_total, 2,',',' ').'','1','0','R');
    $pdf->Ln(20);
