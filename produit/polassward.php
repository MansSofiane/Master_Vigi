...
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();	
	
}else{
//Calcule du nombre de page 
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();
			...
			<tr class="gradeX">
			 <?php if($row_res['etat']==0){ ?>
				...
				<td><?php echo $row_res['annee']; ?></td>
				 ...
				 <?php if($row_res['etat']==0){ ?> 
				  &nbsp;&nbsp;&nbsp;
		         <a onClick="vavade2('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['cod_prod'];?>','<?php echo $row_res['ndat_eff'];?>','avpolassward.php','polassward.php')" title="Avenants"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>
		         <?php } ?>
		         &nbsp;&nbsp;&nbsp;
				 ...
				 <a onClick="reglement('<?php echo $row_res['cod_pol'];?>','0','<?php echo $row_res['cod_pol'];?>','polassward.php')" title="Regler"><i CLASS="icon-money icon-2x" style="color:green"  /></a>&nbsp;&nbsp;&nbsp;
				 <?php }?>
				 <a onClick="lavis('<?php echo $row_res['cod_pol'];?>','avispolassward.php','<?php echo $row_res['sequence'];?>')" title="Liste-Avis-recette"><i CLASS="icon-list icon-2x" style="color:lightseagreen"/></a>
