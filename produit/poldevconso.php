...
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous`  AND d.`cod_prod`='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
    $part=$page*7;
//requete à suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();

}else{
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
    $part=$page*7;
//requete à suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
    $nb = $rqt->execute();
}
?>
...
<div class="widget-box">
    <ul class="quick-actions">
        ...
        <li class="bg_lb"> <a onClick="Menu1('prod','devconso.php')"> <i class="icon-folder-open"></i>Visualiser-Devis</a></li>
        <li class="bg_lg"> <a onClick="Menu1('prod','poldevconso.php')"> <i class="icon-folder-open"></i>Visualiser-Contrats</a> </li>
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <a href="sortie/contrat6/<?php echo crypte($row_res['cod_pol']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
                        <?php if($row_res['etat']==0){ ?>
                            &nbsp;&nbsp;&nbsp;
                            <a onClick="vav1('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['dat_eff'];?>','poldevconso.php')" title="Retrait"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>
                        <?php } ?>
                        &nbsp;&nbsp;&nbsp;
                        <a onClick="lav('<?php echo $row_res['cod_pol'];?>','avpolconso.php')" title="Liste-Retraits"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
                       &nbsp;&nbsp;&nbsp;
...
<script language="JavaScript">
    function frechpade(){
        ...
        $("#content").load('produit/poldevconso.php?rech='+rech+'&crit='+crit);
    }
    function frechpade2(){
        var rech='';
        var crit=2;
    ...
    function fpagepade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load('produit/poldevconso.php?page='+page+'&rech='+rech+'&crit='+crit);
                else
                    $("#content").load('produit/poldevconso.php?page='+page);
            }
        }else{swal("Vous etes en premiere page !");}
    }
   
    function ndev(){
        $("#content").load('produit/ade/conso/sous.php');
    }
	
	function Export(code)
    {
        window.open('excel/Excel-line/'+code+'/4' ,"Generation Excel","menubar=no, status=no, scrollbars=yes, menubar=no, width=600, height=230");
