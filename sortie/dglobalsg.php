...
// Instanciation de la classe derivee
    $pdf->Cell(380,10,'Etat récapitulatif de la production Bancassurance ' ,'','1','C');
    ...
    if ($agence == '0') {
        ...
        $pdf->Cell(210, 10, 'Direction Régionale: ' . $dre, '1', '0', 'C');
    ...
    {
        $pdf->Cell(210, 10, 'AgenceN° ' . $agence, '1', '0', 'C');
        $pdf->Cell(170, 10, 'Produit: Tous les produits ', '1', '0', 'C');
       // $pdf->Cell(130, 10, 'Code produit: ', '1', '1', 'C');
    }
    $pdf->Ln(15);
    ...
    $rqt_prod=$bdd->prepare("SELECT * FROM `produit` WHERE `cod_prod` in (1,2,5,6,7,9); ");
...
group by table1.cod_prod");
        ...
        $pdf->Cell(40,28,''.$produits,'1','0','C');
        $pdf->Cell(50,7,'Production(+)','1','0','C');
        $pdf->Cell(20,7,''.$nb_act_positive_pi,'1','0','C');
        $pdf->Cell(40,7,''.number_format($prime_nette_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,7,''.number_format($Accessoire_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($prime_commerciale_positive_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,7,''.number_format($dt_positifs_pi, 2,',',' ').'','1','0','R');
        ...
        $pdf->Cell(40,7,''.number_format($solde_total_positif_pi, 2,',',' ').'','1','0','R');$pdf->Ln(7);
        $X=$pdf->GetX();
        $pdf->SetX($X+40);

        // $pdf->Cell(50,10,''.$produits,'1','0','C');
        $pdf->Cell(50,7,'Avenant (-) Avec ristourne','1','0','C');
        $pdf->Cell(20,7,''.$nb_act_ar_pi,'1','0','C');
        $pdf->Cell(40,7,''.number_format($prime_nette_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,7,''.number_format($Accessoire_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($prime_commerciale_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,7,''.number_format($dt_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,7,''.number_format($prime_total_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,7,''.number_format($reglement_total_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($solde_total_ar_pi, 2,',',' ').'','1','0','R');
        $pdf->Ln(7);
        $X=$pdf->GetX();
        $pdf->SetX($X+40);
        // $pdf->Cell(50,10,''.$produits,'1','0','C');
        $pdf->Cell(50,7,'Avenant (-) sans ristourne','1','0','C');
        $pdf->Cell(20,7,''.$nb_act_sr_pi,'1','0','C');
        $pdf->Cell(40,7,''.number_format($prime_nette_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,7,''.number_format($Accessoire_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($prime_commerciale_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,7,''.number_format($dt_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,7,''.number_format($prime_total_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,7,''.number_format($reglement_total_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($solde_total_sr_pi, 2,',',' ').'','1','0','R');
        $pdf->Ln(7);
        $X=$pdf->GetX();
        $pdf->SetX($X+40);
        $pdf->SetFont('Arial','B',12);
        ...
        $pdf->Cell(20,7,'','1','0','C');
        $pdf->Cell(40,7,''.number_format($prime_nette_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(30,7,''.number_format($Accessoire_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($prime_commerciale_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(20,7,''.number_format($dt_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,7,''.number_format($prime_total_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(50,7,''.number_format($reglement_total_pi, 2,',',' ').'','1','0','R');
        $pdf->Cell(40,7,''.number_format($solde_total_pi, 2,',',' ').'','1','0','R');
        $pdf->Ln(7);
    ...
    $pdf->SetFillColor(199,139,85);
    $pdf->Cell(320,10,"Direction Régionale Le:    ".date("d/m/Y", strtotime($datesys)) ,'','0','R');  $pdf->Ln(10);
