...
$rqtp->execute();
	$agence="";
	$pdf->Cell(405,10,'Bordereau de production positif du '.date("d/m/Y", strtotime($date1)).' au '.date("d/m/Y", strtotime($date2)).'  --Document généré le-- '.date("d/m/Y", strtotime($datesys)) ,'1','1','L','1');
...
$pdf->Cell(30,5,''.number_format($row_g['mtt_cpl'], 2,',',' ').'','1','0','R');$tcp=$tcp+$row_g['mtt_cpl'];
$pdf->Cell(35,5,''.number_format($row_g['pn']+$row_g['mtt_cpl'], 2,',',' ').'','1','0','R');$tpc=$tpc+($row_g['pn']+$row_g['mtt_cpl']);
$pdf->Cell(20,5,''.number_format($row_g['mtt_dt'], 2,',',' ').'','1','0','R');$tdt=$tdt+$row_g['mtt_dt'];
$pdf->Cell(35,5,''.number_format($row_g['pt'], 2,',',' ').'','1','0','R');$tpt=$tpt+$row_g['pt'];
