...
function dprod(code) {
	...
	if (verifdatestat(date1) && verifdatestat(date2)) {
       ...
       if(code != 100){
		...
		//le nouveau generateur global
		window.open('excel/Excel-PD/' + user + '/' + d1 + '/' + d2+ '/' + code,"Generation Excel","menubar=no, status=no, scrollbars=yes, menubar=no, width=600, height=230");
		}
