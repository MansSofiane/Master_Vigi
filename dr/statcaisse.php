...
$rqt = $bdd->prepare("SELECT distinct agence FROM `utilisateurs`  WHERE id_par='$user'  ORDER BY `agence`");
$rqt->execute();
?>
            ...
            if (verifdatestat(date1) && verifdatestat(date2)) {
                ...
                if (type == 5) {
                    if (code != 100) {
                        ...
                        window.open('sortie/dcaissem/' + d1 + '/' + code + '/' + d2 + '/' + type + '/' + user, 'Finances', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no');
                    }
                    else {
                        window.open('sortie/dcaissegm/' + d1 + '/' + code + '/' + d2 + '/' + type + '/' + user, 'Financesconsolide', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no');
                    }
                } else {
                    if (code != 100) {

                        window.open('sortie/dcaisse/' + d1 + '/' + code + '/' + d2 + '/' + type + '/' + user, 'Finances', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no');
                    }
                    else {
                        window.open('sortie/dcaisseg/' + d1 + '/' + code + '/' + d2 + '/' + type + '/' + user, 'Financesconsolide', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no');
                    }
                }
            }
        }
