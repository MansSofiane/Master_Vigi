...
// Instanciation de la classe derivee
    ...
    $rqtuser=$bdd->prepare("select agence from utilisateurs where id_user='$user'");
    $rqtuser->execute();
    $agence_compte="";
    while($rt=$rqtuser->fetch())
    {
        $agence_compte=$rt['agence'];
    ...
    while ($row_p=$rqtp->fetch()){
        ...
        if($agence_selected!='0')
            $pdf->Cell(405,10,'AgenceN°: '.$agence_selected,'1','0','L');
        else
            $pdf->Cell(405,10,'AgenceN°:  toutes les agences de la DR: '.$agence_compte,'1','0','L');
