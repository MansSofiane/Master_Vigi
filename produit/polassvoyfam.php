...
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous`  AND d.`cod_prod`='1'  and d.cod_formul='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  AND $condition ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence') AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();

}else{
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM `policew`
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
//requete à suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM `policew`
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='4' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
                ...
                <tr class="gradeX">
                   ...
                   <?php if($row_res['rp_sous']==0){
                    ...
                    <td>&nbsp;
                        ...
                        <?php if($row_res['etat']==0){ ?>
                            &nbsp;&nbsp;&nbsp;
                            <a onClick="vavade2('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['cod_prod'];?>','<?php echo $row_res['ndat_eff'];?>','avpolassvoyfam.php','polassvoyfam.php')" title="Avenant"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>
                        <?php } ?>
                        &nbsp;&nbsp;&nbsp;
                        <a onClick="lav('<?php echo $row_res['cod_pol'];?>','avpolassvoyfam.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
                        <?php if ($row_res['mtt_solde']>0){ ?>
                            <a onClick="reglement('<?php echo $row_res['cod_pol'];?>','0','<?php echo $row_res['cod_pol'];?>','polassvoyfam.php')" title="Regler"><i CLASS="icon-money icon-2x" style="color:green"  /></a>&nbsp;&nbsp;&nbsp;
                        <?php }?>
                        <a onClick="lavis('<?php echo $row_res['cod_pol'];?>','avispolassvoyfam.php','<?php echo $row_res['sequence'];?>')" title="Liste-Avis-recette"><i CLASS="icon-list icon-2x" style="color:lightseagreen"/></a>
