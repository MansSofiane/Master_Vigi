...
// Instanciation de la classe derivee
    ...
    if ($agence == '0') {
        $rqt_prod=$bdd->prepare("SELECT distinct agence FROM `utilisateurs` WHERE `type_user`='user' order by agence; ");
        ...
        if ($typ_usr == 'dr') {
            $rqt_prod=$bdd->prepare("SELECT distinct agence FROM `utilisateurs` WHERE `type_user`='user' and  `id_par`='$agence'order by agence; ");

            $rqt_prod->execute();
        }
        else {
            $rqt_prod=$bdd->prepare("SELECT distinct agence FROM `utilisateurs` WHERE `type_user`='user' and  `id_user`='$agence'order by agence; ");
