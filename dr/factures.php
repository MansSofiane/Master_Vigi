...
//Calcule du nombre de page
    $rqtc=$bdd->prepare("select `id_facture`, `sequence`, `dat_facture`, `dat_deb`, `dat_fin`, `mtt_net`, `taux_tva`, `TVA`, `mtt_ttc`, `id_user`, `cod_agence`, `type_facture` from facture where id_user='$id_user' and code_agence ='$rech' ");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/5);
//Pointeur de page
    $part=$page*5;
//requete à suivre
    $rqt=$bdd->prepare("select `id_facture`, `sequence`, `dat_facture`, `dat_deb`, `dat_fin`, `mtt_net`, `taux_tva`, `TVA`, `mtt_ttc`, `id_user`, `cod_agence`, CASE `type_facture` WHEN 0 THEN 'Direct' else'Via-courtier' end typ_facture from facture where id_user='$id_user' and code_agence ='$rech' ORDER BY `dat_facture` DESC LIMIT $part ,5");
    $rqt->execute();

}else{
//Calcule du nombre de page
    $rqtc=$bdd->prepare("select `id_facture`, `sequence`, `dat_facture`, `dat_deb`, `dat_fin`, `mtt_net`, `taux_tva`, `TVA`, `mtt_ttc`, `id_user`, `cod_agence`, `type_facture` from facture where id_user='$id_user' ");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/5);
//Pointeur de page
    $part=$page*5;
//requete à suivre
    $rqt=$bdd->prepare("select `id_facture`, `sequence`, `dat_facture`, `dat_deb`, `dat_fin`, `mtt_net`, `taux_tva`, `TVA`, `mtt_ttc`, `id_user`, `cod_agence`, CASE `type_facture` WHEN 0 THEN 'Direct' else'Via-courtier' end typ_facture from facture where id_user='$id_user' ORDER BY `dat_facture` DESC  LIMIT $part ,5");
    $rqt->execute();
    $nb = $rqt->execute();
}
?>
...
<div class="widget-box">
    <ul class="quick-actions">
        ...
        <li class="bg_lg"> <a onClick="dMenu1('com','gfacture.php')"> <i class="icon-plus"></i>G&eacute;n&eacute;rer une facture</a> </li>
        <li class="bg_lo"> <a onClick="dMenu1('com','factures.php')"> <i class="icon-folder-open"></i>Factures de commissions</a> </li>
    </ul>
</div>
<div class="widget-box">
    <div class="widget-title">
        <div><input type="text" id="nsousvcpl" onchange="frechvcpld()" class="span4" placeholder="Rechercher par Nom-Souscripteur..."/></div>
    </div>
...
<script language="JavaScript">
    function frechvcpld(){
        ...
        $("#content").load('dr/fcatures.php?rech='+rech);
    }
    function fpagegav(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('dr/fcatures.php?page='+page);}
