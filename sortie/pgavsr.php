...
$query_sous =$bdd->prepare("SELECT s.*,v.*,o.lib_opt, p.lib_pays,d.sequence as sequence2,d.dat_val as dat_valp,t.`mtt_dt`,c.`mtt_cpl`,pr.code_prod FROM `souscripteurw` as s, `policew` as d,`avenantw` as v, `option` as o, `pays` as p,produit as pr, `dtimbre` as t , `cpolice` as c WHERE v.`cod_dt`=t.`cod_dt` AND v.`cod_cpl`=c.`cod_cpl` AND s.cod_sous=d.cod_sous and d.cod_prod=pr.cod_prod and v.cod_prod=pr.cod_prod and d.cod_pol=v.cod_pol and v.cod_opt=o.cod_opt and p.cod_pays=v.cod_pays and v.cod_av='".$row."';");
$query_sous->execute();
...
// Voyage
        ...
        $dat_val=date("d/m/Y", strtotime($row_sous['dat_val']));
...
while($rowq=$rqtseq->fetch())
{
    ...
    $pdf->Cell(100,20,'     DU:'.$dat_val.'                  ','RB','0','R');$pdf->Ln();
